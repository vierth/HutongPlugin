...
struct FHutongHallParams
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Hall", meta=(DisplayName="Base Course Height (下鹼)", UIMin="0", UIMax="200", ClampMin="0", Units="cm", ToolTip="Height of the base course (下鹼) above the floor, in cm. Zero derives it so the band tops out at the canon line above the ground, the same line every piece of a frontage shares."))
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Bays", meta=(DisplayName="Min Bay Width", UIMin="200", UIMax="500", ClampMin="100", Units="cm", ToolTip="Smallest bay width allowed when deriving the bay count from the frontage, in cm."))
	double MinBayWidth = 300.0;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Bays", meta=(DisplayName="Max Bay Width", UIMin="250", UIMax="600", ClampMin="120", Units="cm", ToolTip="Largest bay width allowed when deriving the bay count from the frontage, in cm."))
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Bays", meta=(DisplayName="Column Taper (收分)", UIMin="0", UIMax="0.02", ClampMin="0", ClampMax="0.05", ToolTip="Column taper (收分) as the fraction of column height lost from the diameter at the head."))
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Bays", meta=(DisplayName="Bracket Reach", EditCondition="bHasBrackets", UIMin="20", UIMax="90", ClampMin="5", Units="cm", ToolTip="How far each sparrow brace (雀替) extends along the beam from its column, in cm."))
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Facade", meta=(DisplayName="Sill Height Above Floor (檻牆)", UIMin="60", UIMax="120", ClampMin="20", Units="cm", ToolTip="Height of the sill wall (檻牆) under the lattice windows above the floor, in cm."))
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Roof", meta=(DisplayName="Purlins (檁數)", ToolTip="Number of purlins (檁) across the roof section; sets the suggested depth and the roof facets."))
	EHutongPurlins Purlins = EHutongPurlins::Seven;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Roof", meta=(DisplayName="Apex Roll (捲棚)", UIMin="0", UIMax="0.6", ClampMin="0", ClampMax="1", ToolTip="How much the roof apex is rounded into a rolled ridge (捲棚) crown; 0 keeps the ridge fold."))
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Roof", meta=(DisplayName="Bargeboard Depth (博風板)", EditCondition="RoofType == EHutongRoofType::Xieshan", UIMin="0", UIMax="60", ClampMin="0", Units="cm", ToolTip="Vertical depth of the bargeboard (博風板) along each gable rake, in cm."))
	...
	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Roof", meta=(DisplayName="Corner Flare Rise (翼角)", UIMin="0", UIMax="140", ClampMin="0", Units="cm", ToolTip="How far each eave corner lifts for the upturned corner (翼角) flare, in cm."))
	double RoofFlareRise = 62.0;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Roof", meta=(DisplayName="Corner Flare Run", UIMin="0", UIMax="120", ClampMin="0", Units="cm", ToolTip="How far each eave corner pushes outward for the upturned corner (翼角) flare, in cm."))
	double RoofFlareRun = 46.0;

	UPROPERTY(EditAnywhere, BlueprintReadWrite, Category="Roof", meta=(DisplayName="Eave Fascia Depth (勾頭滴水)", UIMin="0", UIMax="35", ClampMin="0", Units="cm", ToolTip="Vertical depth of the eave caps and drip tiles (勾頭滴水) fascia band along the eave, in cm."))
